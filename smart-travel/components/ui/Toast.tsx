...
const Toast: React.FC<ToastProps> = ({ id, message, type, onClose, duration = 5000 }) => {
  ...
  return (
    <div
      id={`toast-${type}-${id}`}
      className="flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow animate-fade-in-right"
      role="alert"
    >
      <div className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-lg ${TYPE_CLASSES[type]}`}>
        {ICONS[type]}
